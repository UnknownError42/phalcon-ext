...
class ModelGenerator
{
     ...
     * @param $prefix
     */
    public function __construct(string $namespace, string $dir, string $prefix)
    {
    ...
    {
        $di = Di::getDefault();
        $this->db = $di->get($connectionService);
    }
...
php;
    ...
    public function generateBaseModel($tableName)
    {
        ...
        foreach ($columns as $column) {
            $attributes[$column->getName()] = $column->hasDefault() ? $column->getDefault() : null;
            //生成验证信息
            ...
            switch ($column->getType()) {
                ...
                case Column::TYPE_TIMESTAMP:
                    ...
                    $attributes[$column->getName()] = null;
                    break;
...
php;
        ...
        $finalCode = $this->renderClass($this->namespace . '\Base', $className, 'Model', $this->renderAttributes($attributes), $this->renderMethods($methods));
        if (!file_exists(Text::concat("/", $this->modelsDir, 'base'))) {
            mkdir(Text::concat("/", $this->modelsDir, 'base'), 0755, true);
        }
        $filename = Text::concat("/", $this->modelsDir, 'base', "{$className}.php");
    ...
    public function generateChildModel($tableName)
    {
        ...
        $baseClassName = "Base\Base{$className}";
...
eot;
    ...
    public function createModels()
    {
        ...
        foreach ($tables as $table) {
            if ($this->prefix === '' || Text::startsWith($table, $this->prefix)) {
